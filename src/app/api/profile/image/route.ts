...
function getBackendErrorMessage(responseBody: unknown) {
  ...
  return "Unable to upload the profile image.";
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Invalid profile image upload payload." },
      { status: 400 },
    );
  }
  ...
  try {
    const response = await fetch(`${apiBaseUrl}${apiEndpoints.auth.profileImage}`, {
      method: "POST",
      headers: {
