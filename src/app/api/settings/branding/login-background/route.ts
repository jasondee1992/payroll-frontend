...
function getBackendErrorMessage(responseBody: unknown) {
  ...
  return "Unable to upload the login background.";
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Invalid login background upload payload." },
      { status: 400 },
    );
  }
  ...
  if (!(image instanceof File)) {
    return NextResponse.json(
      { error: "Login background image file is required." },
      { status: 400 },
    );
  }
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/api/v1/branding/login-background`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({
      branding,
      assetUrl: resolveBrandingAssetUrl(branding.loginBackgroundPath),
    });
  } catch {
    return NextResponse.json(
